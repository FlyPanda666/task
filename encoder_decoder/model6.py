...
class MultiHeadAttention(nn.Module):
    def __init__(self, hidden_dim, num_attention_heads, dropout, device):
        super().__init__()
...
class EncoderLayer(nn.Module):
    def __init__(self, hidden_dim, num_attention_heads, intermediate_dim, dropout, device):
        super().__init__()
        self.self_attention_layer_norm = nn.LayerNorm(hidden_dim)
        self.position_wise_feed_forward_layer_norm = nn.LayerNorm(hidden_dim)
        self.self_attention_layer = MultiHeadAttention(hidden_dim, num_attention_heads, dropout, device)
        self.position_wise_feed_forward_layer = PositionWiseFeedforwardLayer(hidden_dim, intermediate_dim, dropout)
        self.dropout = nn.Dropout(dropout)

    def forward(self, src, src_mask):
        _src, _ = self.self_attention_layer(src, src, src, src_mask)
        src = self.self_attention_layer_norm(src + self.dropout(_src))
        _src = self.position_wise_feed_forward_layer(src)
        src = self.position_wise_feed_forward_layer_norm(src + self.dropout(_src))
        return src


class Encoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, num_attention_heads,
                 intermediate_dim, dropout, device, max_length=100):
        super().__init__()
        self.device = device
        self.token_embedding = nn.Embedding(input_dim, hidden_dim)
        self.position_embedding = nn.Embedding(max_length, hidden_dim)
        self.layers = nn.ModuleList([EncoderLayer(hidden_dim, num_attention_heads, intermediate_dim, dropout, device)
