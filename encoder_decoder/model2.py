...
class Encoder(nn.Module):
    def __init__(self, input_dim, embedding_dim, hidden_dim, dropout):
        ...
        self.rnn = GRU(input_size=embedding_dim, hidden_size=hidden_dim)
...
class Decoder(nn.Module):
    def __init__(self, output_dim, embedding_dim, hidden_dim, dropout):
        ...
        self.rnn = GRU(input_size=embedding_dim + hidden_dim, hidden_size=hidden_dim)
        self.fc_out = nn.Linear(embedding_dim + 2 * hidden_dim, output_dim)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, inputs, hidden, context):
        ...
        embedding_concat = torch.cat((inputs_embed, context), dim=2)
        outputs, hidden = self.rnn(embedding_concat, hidden)
        outputs = torch.cat((inputs_embed.squeeze(0), hidden.squeeze(0), context.squeeze(0)), dim=1)
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, target, teacher_forcing_ratio=0.5):
        ...
        for t in range(1, target_len):
            ...
            prediction, hidden = self.decoder(inputs, hidden, context)
