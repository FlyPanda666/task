...
class AdaptiveAdditionPredictor(nn.Module):
    ...
    def forward(self, query, context, mask):
        ...
        """
        # [b, c, t, e]
        context_ = context.unsqueeze(1).expand(context.size(0), query.size(0), context.size(1), context.size(2))
        ...
        pred = self.v(torch.tanh(self.hidden(
            torch.cat([query, g, torch.abs(query - g), query * g], dim=-1)))).squeeze(-1)  # [b, c]
        return pred
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, key, value, query, mask):
        ...
        query, key, value = [l(x).view(batch_size, -1, heads_num, per_head_size).transpose(1, 2)
