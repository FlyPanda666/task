...
class BertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.multi_head_attention = attention(embed_dim=config.hidden_size,
    ...
    def forward(self, query, key, value, attn_mask=None, key_padding_mask=None):
        ...
        在Bert中attention_mask指代的其实是key_padding_mask,因为Bert主要是基于Transformer Encoder部分构建的，
...
class BertAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None):
        ...
        self_outputs = self.self(
            hidden_states, hidden_states, hidden_states, attn_mask=None, key_padding_mask=attention_mask)
...
def replace_512_position(init_embedding, loaded_embedding):
    """本函数的作用是当max_positional_embedding > 512时,用预训练模型中的512个向量来替换随机初始化的positional embedding中的前512个向量
            ...
            for i in range(len(model_paras_names)):
                ...
                if "position_embeddings" in model_paras_names[i]:
                    ...
                    if config.max_position_embeddings > 512:
                        new_embedding = replace_512_position(
