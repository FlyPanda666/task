...
def read_data(filename: str) -> str:
    """ 数据读取
    :param filename: 文件路径，这里还是传入的参数包含文件路径比较好，可以实现通用的效果。
    :return: 数据读取内容（整个文档的字符串）
    """
    with open(filename, mode="r", encoding="utf8") as reader:
        content = reader.read()
    return content
...
def convert_text2index(sentence: str, word2index: dict, max_length=config.LARGE_SENTENCE_SIZE) -> list:
    """ 将语料转成数字化数据
    :param sentence: 单条文本
...
def number_sentence(pos_text: str, neg_text: str, word2index: dict) -> np:
    """ 语句数字化处理
...
def get_batch(x: np, y: np, batch_size=config.BATCH_SIZE, shuffle: bool = True) -> tuple:
    """ 构建迭代器，获取批次数据
