...
def load_data():
    print('loading data...\n')
    data = pickle.load(open(opt.data + 'data_teacher_yaqiang.pkl', 'rb'))
...
def eval_model(model, data):
    ...
    if config.unk and config.attention != 'None':
        ...
        candidate = cands
    print("开始写结果")
    write_data(candidate, reference)
    results = utils.eval_metrics(reference, candidate, label_dict, opt.log)
    results = [('%s: %.5f' % item + '\n') for item in results.items()]
    with codecs.open(opt.log + 'results_teacher_yaqiang1.txt', 'w', 'utf-8') as f:
        f.writelines(results)


def write_data(cand, ref):
    cand = [" ".join(item) for item in cand]
    ref = [" ".join(item) for item in ref]
    data = {"predicted": cand, "labeled": ref}
    df = pd.DataFrame(data)
    df.to_csv("./result_teacher_yaqiang1.csv", index=False)
