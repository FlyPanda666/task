...
def set_args():
    ...
    parser.add_argument('--model_path', default='./output_dir/checkpoint-117446', type=str, help='模型文件路径')
    parser.add_argument('--vocab_path', default='./vocab/vocab.txt', type=str, help='词表，该词表为小词表，并增加了一些新的标记')
    parser.add_argument('--batch_size', default=5, type=int, help='生成标题的个数')  # 原来是3个
    parser.add_argument('--generate_max_len', default=100, type=int, help='生成标题的最大长度')
    ...
    parser.add_argument('--max_len', type=int, default=400, help='输入模型的最大长度，要比config中n_ctx小')  # 原来是512
    return parser.parse_args()


def top_k_top_p_filtering(
        logits: torch.Tensor, top_k: int, top_p: float, filter_value: float = -float("Inf")) -> torch.Tensor:
...
def predict_one_sample(
        model: GPT2LMHeadModel, tokenizer: BertTokenizer, device: torch.device, args: argparse, content: str):
    """预测样本的标签.
    :param model:
    :param tokenizer:
    :param device:
    ...
    next_token_type = torch.tensor([[label_id] for _ in range(args.batch_size)]).long().to(device)
    generated = []
    finish_set = set()
...
def main():
    ...
    model.to(device)
    model.eval()
    valid_data_path = "./data_dir/dev_data_grade3.json"
    ...
    for line in content:
        ...
        answer.append({"content": line["content"], "que_id": line["title"], "labels": labels})
        cnt += 1
        if cnt % 1000 == 0:
            print(f"已经预测{cnt}个试题的标签")
    output_dir = "./dev_result.xlsx"
